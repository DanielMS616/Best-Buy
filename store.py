...
def main():
    """Test the Store class with the example from the assignment."""

    # Create three Product objects that form the initial inventory.
    product_list = [
        products.Product(
            "MacBook Air M2",
            price=1450,
    ...
    # get_all_products() returns a new list containing only the
    # Product objects that are currently active.
    active_products = best_buy.get_all_products()

    # The initial total quantity is 100 + 500 + 250 = 850.
    print(best_buy.get_total_quantity())

    # The list passed to order() contains two tuples:
    # index 0 refers to the MacBook Product object, quantity 1;
    # index 1 refers to the Bose Product object, quantity 2.
    print(
        best_buy.order(
            [
                (active_products[0], 1),
    ...
    # Three items were purchased in total, so the remaining stock
    # quantity should be 850 - 3 = 847.
    print(best_buy.get_total_quantity())
